...
Status CatalogManager::SetWalRetentionForTable(
  ...
  const auto retention_to_set = GetAtomicFlag(&FLAGS_cdc_wal_retention_time_secs);
  {
    auto l = table->LockForRead();
    ...
    if (l->has_wal_retention_secs() && l->wal_retention_secs() >= retention_to_set) {
      VLOG_WITH_FUNC(1) << "Skipping since table " << table_id
                        << " already has a sufficient WAL retention of " << l->wal_retention_secs();
      return Status::OK();
    }
  }
  ...
  alter_table_req.set_wal_retention_secs(retention_to_set);

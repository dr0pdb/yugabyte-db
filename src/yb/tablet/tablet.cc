...
Status Tablet::ApplyOperation(
  ...
  if (FLAGS_TEST_override_op_type_for_raft) {
    if (!operation.is_local() && operation.isLeaderSide()) {
      LOG(INFO) << __func__
                << ": Skipping ApplyOperation for the remote op on the leader for request: "
                << operation.request()->ShortDebugString();
      return Status::OK();
    }

    LOG(INFO) << __func__
              << ": Going ahead with the ApplyOperation with is_local: " << operation.is_local()

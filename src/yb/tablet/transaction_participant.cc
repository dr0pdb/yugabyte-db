 ...
 private:
  ...
  LockAndFindResult LockAndFind(
              ...
              << GetStackTrace(); */
    {
      ...
      auto it = transactions_.find(id);
      if (it != transactions_.end()) {
        // LOG(INFO) << __func__ << " RKNRKN found the transaction " << id << "in transactions
        // list";
    ...
    if (recently_removed) {
      ...
      /* LOG(INFO) << __func__ << " RKNRKN Attempt to load recently removed transaction: " << id
                << ", for: " << reason; */
      return LockAndFindResult{};
    }
  ...
  void RemoveTransaction(Transactions::iterator it, RemoveReason reason,
      ...
      REQUIRES(mutex_) {
    auto now = CoarseMonoClock::now();
    /* LOG(INFO) << __func__ << " RKNRKN about to cleanup the transaction with id "
              << (**it).ToString(); */

...
Status OperationDriver::Init(std::unique_ptr<Operation>* operation, int64_t term) {
  ...
  if (term == OpId::kUnknownTerm) {
    ...
    if (consensus_) {  // sometimes NULL in tests
      if (FLAGS_TEST_override_op_type_for_raft && operation_->operation_mode() == OperationMode::kLocal) {
...
void OperationDriver::ReplicationFinished(
  ...
  LOG_IF(DFATAL, status.ok() && !GetOpId().valid() && !FLAGS_TEST_override_op_type_for_raft) << "Invalid op id after replication";
  TRACE_FUNC();
...
void OperationDriver::ApplyTask(int64_t leader_term, OpIds* applied_op_ids) {
  ...
  if (FLAGS_TEST_override_op_type_for_raft && applied_op_ids == nullptr) {
    LOG(INFO) << __func__
              << " called with nullptr applied ops for request: " << operation_->request()->ShortDebugString();
  }

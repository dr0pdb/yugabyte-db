...
Status PopulateCDCSDKIntentRecord(
  ...
  TableId table_id = tablet->metadata()->table_id();
  Slice prev_key;
...
Status PopulateCDCSDKWriteRecord(
  ...
  TableId table_id = tablet_ptr->metadata()->table_id();
...
Status ProcessIntents(
  ...
  if (!keyValueIntents->empty()) {
    RETURN_NOT_OK(PopulateCDCSDKIntentRecord(
        ...
        &reverse_index_key, commit_time, client));
  }
...
Status PopulateCDCSDKSafepointOpRecord(
    const uint64_t timestamp, const string& table_name, const TableId& table_id,

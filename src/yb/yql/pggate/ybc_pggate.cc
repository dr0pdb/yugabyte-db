...
YBCStatus YBCPgGetReplicationSlot(
  ...
  LOG(INFO) << "Slot info from pg_client_service = " << slot_info.DebugString();
  ...
  *replication_slot =
  ...
  LOG(INFO) << "done with getting slot";
  return YBCStatusOK();
}
...
YBCStatus YBCPgGetTabletListToPollForStreamAndTable(const char *stream_id,
                                                    ...
                                                    size_t *numtablets) {
  LOG(INFO) << "YBCPgGetTabletListToPollForStreamAndTable start";
  const PgObjectId pg_table_id(database_oid, table_oid);
  LOG(INFO) << "YBCPgGetTabletListToPollForStreamAndTable for table = " << pg_table_id
            << " and stream_id = " << std::string(stream_id);
  const auto result = pgapi->GetTabletListToPollForCDC(std::string(stream_id), pg_table_id);
  if (!result.ok()) {
    LOG(INFO) << "YBCPgGetTabletListToPollForStreamAndTable got error " << result.status();
    return ToYBCStatus(result.status());
  }

  LOG(INFO) << "tablet_checkpoint_pairs from cdc_service = " << result.get().DebugString();
  ...
  *numtablets = tablet_checkpoint_pairs.size();
  ...
  LOG(INFO) << "Populated all tablet checkpoints";
  return YBCStatusOK();
}
...
YBCStatus YBCPgSetCDCTabletCheckpoint(
  ...
  LOG(INFO) << "YBCPgSetCDCTabletCheckpoint for tablet = " << std::string(tablet_id)
            << " and stream_id = " << std::string(stream_id);
  ...
  if (!result.ok()) {
    LOG(INFO) << "YBCPgSetCDCTabletCheckpoint got error " << result.status();
    return ToYBCStatus(result.status());
  }

  LOG(INFO) << "SetCDCTabletCheckpoint response from cdc_service = " << result.get().DebugString();
  return YBCStatusOK();
}
...
YBCStatus YBCPgGetCDCChanges(
  ...
  LOG(INFO) << "YBCPgGetCDCChanges for tablet = " << std::string(tablet_id)
            << " and stream_id = " << std::string(stream_id);
  const auto result =
      pgapi->GetCDCChanges(std::string(stream_id), std::string(tablet_id), checkpoint);
  if (!result.ok()) {
    LOG(INFO) << "YBCPgGetCDCChanges got error " << result.status();
    return ToYBCStatus(result.status());
  }
  LOG(INFO) << "GetChanges response from cdc_service = " << result.get().DebugString();
  ...
  *DCHECK_NOTNULL(record_batch) = NULL;
  ...
  for (const auto& row_pb : resp_rows_pb) {
    ...
    // TODO: Populate the rest of the values such as table. Sending dummy values for now.
    new (&resp_rows[row_idx]) YBCPgRowMessage{
        .col_count = col_count,
        .cols = cols,
        .commit_time = commit_time,
        .transaction_id = 1,
        .record_op_id_index = static_cast<uint64_t>(row_pb.cdc_sdk_op_id().index()),
  ...
  *record_batch = static_cast<YBCPgChangeRecordBatch *>(YBCPAlloc(sizeof(YBCPgChangeRecordBatch)));
  ...
  LOG(INFO) << "Sending GetChanges response back to PG layer";

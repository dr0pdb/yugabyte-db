...
PgCreateDatabase::PgCreateDatabase(const PgSession::ScopedRefPtr& pg_session,
  ...
  req_.set_uses_regular_transaction_block(uses_regular_transaction_block);
  if (yb_clone_info) {
...
PgCreateTablegroup::PgCreateTablegroup(
    ...
    const PgOid tablegroup_oid, const PgOid tablespace_oid, bool uses_regular_transaction_block)
    : BaseType(pg_session) {
  req_.set_database_name(database_name);
  req_.set_uses_regular_transaction_block(uses_regular_transaction_block);
...
PgCreateTable::PgCreateTable(
    ...
    : BaseType(
          ...
          old_relfilenode_oid, is_truncate, use_transaction, uses_regular_transaction_block) {}

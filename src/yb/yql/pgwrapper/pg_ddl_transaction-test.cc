...
class PgDdlSavepointMiniClusterTest : public PgMiniTestBase, public ::testing::WithParamInterface<TestCommit> {
  protected:
  void SetUp() override {
    ANNOTATE_UNPROTECTED_WRITE(FLAGS_TEST_ysql_yb_ddl_savepoint_enabled) = true;
    ANNOTATE_UNPROTECTED_WRITE(FLAGS_ysql_yb_ddl_transaction_block_enabled) = true;

    // Disable READ_COMMITTED isolation because it creates additional savepoints (sub_transactions)
    // for a statement which requires special handling when asserting the size of
...
TEST_P(PgDdlSavepointMiniClusterTest, TestTransactionStateMultipleSubTransactions) {
  ...
  ASSERT_EQ(table->LockForRead()->ysql_ddl_txn_verifier_state().size(), 1);
  ...
  ASSERT_EQ(table->LockForRead()->ysql_ddl_txn_verifier_state().size(), 2);
  ...
  auto new_table = catalog_mgr.GetTableInfo(new_table_id);
  ...
  ASSERT_TRUE(table->LockForRead()->is_being_created_by_ysql_ddl_txn());
  ASSERT_TRUE(table->LockForRead()->is_being_altered_by_ysql_ddl_txn());
  ASSERT_TRUE(table->LockForRead()->is_being_deleted_by_ysql_ddl_txn());

  // 'new_table' must have only 1 ddl verification state
  auto new_table_verifier_states = new_table->LockForRead()->ysql_ddl_txn_verifier_state();
  ASSERT_EQ(new_table_verifier_states.size(), 1);
  ASSERT_TRUE(new_table_verifier_states[0].contains_create_table_op());
  ASSERT_TRUE(new_table->LockForRead()->is_being_created_by_ysql_ddl_txn());
  ASSERT_FALSE(new_table->LockForRead()->is_being_altered_by_ysql_ddl_txn());
  ASSERT_FALSE(new_table->LockForRead()->is_being_deleted_by_ysql_ddl_txn());

  if (GetParam()) {
    ASSERT_OK(conn.Execute("COMMIT"));
    ASSERT_FALSE(table->LockForRead()->has_ysql_ddl_txn_verifier_state());
    ASSERT_FALSE(new_table->LockForRead()->has_ysql_ddl_txn_verifier_state());
  } else {
    ASSERT_OK(conn.Execute("ROLLBACK"));
  }
}

// Test multiple sub-transactions with DDLs which roll forward such as Drop Column and Drop Table.

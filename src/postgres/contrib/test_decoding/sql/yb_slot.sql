...
-- YB NOTE: We don't support temporary slots currently (#19263).
...
-- YB NOTE: It fails because the creation of the slot itself failed as we don't support temporary
-- slots currently (#19263).
...
-- YB NOTE: We are creating a permanent replication slot here since we don't yet support tempoary
-- slots.
-- YB_TODO(#19263): Must be updated back to temporary once supported.
-- test switching between slots in a session
SELECT 'init' FROM pg_create_logical_replication_slot('regression_slot1', 'test_decoding', false);
...
-- slots.
-- YB_TODO(#19263): Must be updated back to temporary once supported.
SELECT 'init' FROM pg_create_logical_replication_slot('regression_slot2', 'test_decoding', false);
...
-- YB NOTE: We converted these replication slots to be permanent since temporary ones aren't
-- supported yet. So below statements don't fail.

...
WITH test_user_oid AS (
    SELECT oid AS rol_oid FROM pg_roles WHERE rolname = 'test_user'
),
profile3_oid AS (
    SELECT oid AS prf_oid FROM pg_yb_profile WHERE prfname = 'profile_3_failed'
)
UPDATE pg_catalog.pg_yb_role_profile
SET rolprfstatus = 'l',
    rolprffailedloginattempts = 4
WHERE rolprfrole = (SELECT rol_oid FROM test_user_oid)
  AND rolprfprofile = (SELECT prf_oid FROM profile3_oid);

...
typedef struct YbDdlTransactionState
{
	 ...
	 * change can never happen in a transaction block. Thus, it is not possible
	 * for nesting_level > 0 and uses_regular_txn_block == true.
	 */
...
void YBSetDdlState(YbDdlMode mode)
{
...
YBCommitTransactionContainingDDL()
{
	...
	if (uses_regular_txn_block)
		HandleYBStatus(YBCPgCommitPlainTransactionContainingDDL(
					   ...
					   QueryCompletion *qc)
{
	...
	PG_TRY();
	{
		if (is_ddl)
		{
			 ...
			 * yb_ddl_transaction_block_enabled is false or if this is an online

		...
		if (strcmp(error.hint, "Database might have been dropped by another user") == 0)
		{
			...
			if (yb_external_client != NULL &&
				((od_client_t *)yb_external_client)->type == OD_POOL_CLIENT_EXTERNAL)
					od_frontend_error(yb_external_client, KIWI_CONNECTION_DOES_NOT_EXIST,
	...
	while (1) {
		...
		case KIWI_BE_PARAMETER_STATUS: {
			...
			if (is_authenticating)
			{
				 ...
				 */
				if ((((name_len == sizeof("server_encoding") &&
					   strncmp(name, "server_encoding", name_len) == 0)) || ((name_len == sizeof("is_superuser") &&
					   strncmp(name, "is_superuser", name_len) == 0)) || ((name_len == sizeof("session_authorization") &&
					   strncmp(name, "session_authorization", name_len) == 0))))
					break;
